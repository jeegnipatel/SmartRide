...
def searchRider(request):
	...
	for r in riderSet:
		for point in driverRoutePoints:		
			...
			print("point, pickup",point, r.pickUp )
			my_dist = gmaps.distance_matrix(point, r.pickUp)['rows'][0]['elements'][0]["distance"]["text"]
			my_dist = int(re.search(r'\d+', my_dist).group(0))

			print("my_dist", float(my_dist))
			my_dist = float(my_dist)/1000.0
...
def driverQuestions(request):
    print(request.user.username)
    return render(request, "driverSideQuestions.html", {'username': request.user.username})


def driverAnswer(request):
    if request.method == "POST":
        ...
        workstudent = request.POST.get('workstudent')
        con = sqlite3.connect("db.sqlite3")
        cur = con.cursor()
        values = (userid, driverid, talkative, timedriving, music, male, female, workstudent)
        cur.execute("insert into driverquestion values (?, ?, ?, ?, ?, ?, ?, ?)", values)
        con.commit()
        return render(request, "driverHome.html", {'username': request.user.username})
